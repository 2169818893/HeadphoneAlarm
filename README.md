...
默认等待耳机接入、也可自行选择震动或允许外放的 Android 闹钟。
...
耳机闹钟的默认做法是：**只在确认耳机输出后响铃**。它会在响铃前先确认耳机是否在线，并在整个响铃过程中持续校验音频真正的输出设备；若路由不再是可识别的耳机，则静音等待。用户也可以为单条闹钟明确选择“允许外放”，此时不启用仅耳机限制。
...
| 层级 | 手段 | 作用 |
| --- | --- | --- |
| 1 | 耳机模式播放前必须存在耳机设备 | 没有可识别的耳机就不创建音频流，避免误外放 |
...
- 有线耳机 / 带麦耳麦
- 系统明确报告为 `TYPE_USB_HEADSET` 的 Type-C / USB 耳机
- 蓝牙 A2DP / SCO
- Android 12+ 的 BLE 耳机，Android 9+ 的助听器设备

通用的 `TYPE_USB_DEVICE` 也可能是 USB 音箱或 DAC，不能安全地当作耳机；部分被系统仅报告为此类型的无麦 Type-C 耳机将保持静音。只要目标输出被识别为耳机，音频 `usage` 就使用 `USAGE_MEDIA`：`USAGE_ALARM` 可能被机型强制留在机身扬声器。明确选择扬声器播放时才使用 `USAGE_ALARM`。

### 无耳机时的策略

每条闹钟可独立选择没有耳机时的行为：

- **静默等待**：保持静音并监听，可识别的耳机一插入立刻开始播放（默认，无耳机时不出声）
- **仅震动**：只震动提醒，不产生任何声音
- **允许外放**：回退到扬声器播放
...
百分比配置同样按感知（dB）换算成播放器增益：`gain = 10^((p-1)·40/20)`。若把「20%」直接当作线性增益 0.2 使用，实际只衰减约 14dB，听感依然很响；经过 dB 映射后，「上限 20%」的听感才与把系统音量条拉到 20% 相当。

响铃走耳机（媒体流）期间，系统媒体音量会被临时抬到最大、结束后恢复原值。实际听感响度 = 媒体音量 × 播放器增益，若媒体音量停留在平时听歌的水平，渐强会被它封顶、「上限 50% 以上」的设置无法实现；抬满之后闹钟响度才完全由音量上限百分比决定。期间看到音量条被拉满是正常现象，耳机里的实际响度仍由百分比控制。
...
│   └── BootReceiver.kt          开机 / 更新 / 时间或时区变更后恢复闹钟
├── service/
...
- 仅耳机模式的防外放依赖系统如实上报音频路由；部分设备/ROM 可能上报不准，请在实际设备上测试，并为重要日程设置备用闹钟。用户显式选择“允许外放”时可通过扬声器响铃
