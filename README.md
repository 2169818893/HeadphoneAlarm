...
只要目标输出是耳机（有线、USB、蓝牙一律如此），音频 `usage` 就使用 `USAGE_MEDIA`：`USAGE_ALARM` 会被不少机型强制留在机身扬声器（为保证响铃可靠性），导致「戴着耳机也外放」或「耳机无声、误判成外放」的连锁问题；而媒体流在耳机接入后路由到耳机是所有 ROM 的标准行为。回退到扬声器播放时才使用 `USAGE_ALARM`。
